...
class cls_template
{
    ...
    private $_var           = array();
    private $_echash        = '43f312c2fbda602b373ba6c8be3d4b411bf469b2';
    private $_ecsecre       = 'a914ca92fbd74020c2406e567b8cb221cac35d9f028f10ceffec1ec44e8ae787';
...
//            }
        ...
        if (!empty($moddb))
        {
            foreach ($moddb AS $key => $mod)
            {
                ...
                switch ($s[0])
                {
                    ...
                    /* Lấy tên và đường dẫn của file,loại bỏ đuôi */
                    case 'filename':
                        $p = "preg_replace('/\\.[^.\\s]{3,4}$/', '', ".$p.")";
                        break;
    ...
    function smarty_insert_scripts($args)
    {
        ...
        foreach ($arr AS $val)
        {
            if (in_array($val, $scripts) == false)
            {
                ...
                {
                    if(ECS_ADMIN){
                        $baseUrl = str_replace('/'.ADMIN_PATH, '', $this->baseUrl);
                        $val = str_replace('../', '', $val);
                    }
                    $str .= '<script type="text/javascript" src="' . $baseUrl . '/' . $val . '"></script>'."\n";
                }
                else
                {
    ...
    function smarty_prefilter_preCompile($source)
    {
        ...
        if ($file_type == '.dwt')
        {
            ...
            /* 在头部加入版本信息 */
            $source = preg_replace('/<head>/i', "<head><meta name='generator' content='Custom by 0966398663' />",  $source);
    ...
    public function minify_html($input) {
        ...
        /* Tối ưu javascript in html */
        $input = preg_replace_callback('#(.*?)<script(.*?)>(.*?)</script>(.*?)#is',
                    function($matches) {
                        return $matches['1']." <script ".$matches['2'].'>'.$this->minify_js($matches['3'])." </script> ".$matches['4'];
                    }
                ,$input
                );
    ...
    **/
    function smarty_insert_js($args)
    {
        ...
        $nominify = isset($args['no_minify']) && $args['no_minify'] !='' ? explode(',', str_replace(' ', '', $args['no_minify'])) : array();
        $minify   = isset($args['minify']) && $args['minify'] !='' ? explode(',', str_replace(' ', '', $args['minify'])): array();
        $common   = isset($args['common']) && $args['common'] !='' ? explode(',', str_replace(' ', '', $args['common'])): array();
        $mer_js   = array_merge($nominify,$minify);
        $str   = '';
        $file_debug = isset($args['debug']) && $args['debug'] == true ? true : false;
        /* Env Dev */
        if(DEBUG_MODE > 0 || $file_debug){
             foreach ($common as $file) {
                $str .= '<script  type="text/javascript" src="'.$this->baseUrl.'/'.$this->common_path.$file.'"></script>'."\n";
            }
        ...
        /* Env Prodution */
                ...
                foreach ($common as $no) {
                    $file = $this->common_path.$no;
                    if(file_exists(ROOT_PATH.$file)){
                        $ftime = "/* ".date("Y-m-d H:i:s.",filemtime($file))." */";
                        $str .= $ftime.' '.file_get_contents(ROOT_PATH.$file)."\n";
                    }
                }
    ...
    **/
    function smarty_insert_css($args)
    {
        ...
        $file_debug = isset($args['debug']) && $args['debug'] == true ? true : false;
        $common   = isset($args['common']) && !empty($args['common']) ? explode(',', str_replace(' ', '', $args['common'])): array();

        /* Prodution Env */
        if(DEBUG_MODE > 0 || $file_debug){
            foreach ($common AS $val){
                $file = $this->common_path.$val;
                if(file_exists(ROOT_PATH.$file)){
            ...
            foreach ($common AS $val){
                ...
                if(file_exists(ROOT_PATH.$file)){
                    ...
                    $css = $this->makeUlrCss($css, $this->baseUrl.'/'.$this->common_path);
                    $ftime = "/* ".date("Y-m-d H:i:s.",filemtime($file))." */";
                    $str .= $ftime.' '.$css."\n";
                }
            }
