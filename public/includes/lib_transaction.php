...
function get_order_detail($order_id, $user_id = 0)
{
    ...
    /* add by nobj */
    $order['goods_name'] = get_goods_name_by_id($order_id);
        ...
        $order['order_status'] == OS_CONFIRMED))
    {
        ...
        else
        {
            ...
            $order['auto_redirect_pay'] = false; /* add by nobj */
    ...
    $order['how_surplus_name'] = $order['how_surplus'];
    /* 虚拟商品付款后处理 */
    if ($order['pay_status'] != PS_UNPAYED && $payment_info['pay_code'] != 'paymentshop')
    {
        /* edit by Nobj */
        $virtual_goods = get_virtual_goods($order,true,'', true);
        $virtual_card = array();
    ...
    /* 确认时间 支付时间 发货时间 */
    if (isset($order['confirm_time']) && $order['confirm_time'] > 0 && ($order['order_status'] == OS_CONFIRMED || $order['order_status'] == OS_SPLITED || $order['order_status'] == OS_SPLITING_PART))
    {
