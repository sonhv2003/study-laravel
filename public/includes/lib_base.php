...
/* Hàm gửi mail mới */
function sendMail($to,$subject,$content,$bc='',$alt=''){
    include_once(ROOT_PATH . 'includes/cls_email.php');
    $mail = new Email('smtp.gmail.com', 587);
    $mail->setProtocol($mail::TLS)
        ->setLogin($GLOBALS['_CFG']['smtp_user'], $GLOBALS['_CFG']['smtp_pass'])
        ->setFrom($GLOBALS['_CFG']['smtp_mail'], $GLOBALS['_CFG']['shop_name']);
 ...
 * @return boolean
 */
function send_mail($name, $email, $subject, $content, $type = 0, $notification=false, $cc = '')
{
    ...
    else
    {
        ...
        /* add by nobj*/
        if(!empty($cc)){
        $headers[] = 'Cc: "' . '=?' . $charset . '?B?' . base64_encode($name) . '?=' . '" <' . $cc. '>';
        }
