...
/*------------------------------------------------------ */
    ...
    if ($_REQUEST['method'] == 'post')
    {
        ...
        else
        {
            ...
            if(empty($cmt->token) || !isset($cmt->token)){
                $result['error']   = 1;
                $result['message'] = 'Token không hợp lệ';
            }else{
                if($cmt->token =! $_SESSION['cm_csrf_token']){
                    $result['error']   = 1;
                    $result['message'] = 'Token không hợp lệ';
                }
            }

            $_CFG['captcha'] = 0; // Ngắt captcha
