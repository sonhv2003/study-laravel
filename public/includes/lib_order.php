...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0, $rec_type = CART_GENERAL_GOODS, $cform=[])
{
    ...
    if($goods['is_real'] == 0 && $goods['extension_code'] == 'virtual_card'){
        ...
        $virtual_card_num = $GLOBALS['db']->GetOne($sql);
        if ($num > $virtual_card_num)
        {
...
/**
 * Lấy danh sách quà tặng của sản phẩm nếu có
 * @param  [type] $goods_id [description]
 * @return [type]           [description]
 */
function get_gifts_goods($goods_id){
    $now = gmtime();
    ...
    {
        if (strpos(',' . $favourable['act_range_ext'] . ',', ',' . $goods_id . ',') !== false)
        {
            $gift_arr = @unserialize($favourable['gift']);
            foreach ($gift_arr as $row)
            {
                $row['act_id'] = $favourable['act_id'];
                $goods_gift[] = $row;
                /* kiểm tra xem giỏ hàng đã tồn tại quà tặng chưa */

                /**
                 * Kiểm tra xem số lượng quà đã tặng hết chưa
                 * act_type_ext
                 */
                //add_gift_to_cart($act_id, $id, $price);
                /**
                 * Mua 2 thì được tặng 2
                 */
            }
        }
    }
...
function send_order_bonus($order_id)
{
    ...
    if ($bonus_list)
    {
        ...
        if ($count > 0)
        {
            ...
            $res = sendMail($user['email'],$tpl['template_subject'],$content,'',$GLOBALS['_CFG']['shop_name']);
        }
    }
