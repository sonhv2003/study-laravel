...
function assign_ur_here($cat = 0, $str = '', $module='')
{
    ...
    $ur_here    = '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
        ...
        if (!empty($str))
        {
            ...
            $position = $position+1;
            $ur_here    .= '<li itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
                    <a itemprop="item" href="javascript:;"><span itemprop="name">' . $str. '</span></a><meta itemprop="position" content="'. $position.'"></li>';
        }
...
{
    $sql = 'SELECT c.cat_id, c.cat_name, c.sort_order, a.link, a.article_id, a.title, a.file_url, a.open_type ' .
    ...
    foreach ($res AS $key => $row)
    {
        ...
        $arr[$row['cat_id']]['article'][$key]['url']         = build_uri('article', array('aid' => $row['article_id']), $row['title']);
        $arr[$row['cat_id']]['article'][$key]['link_url'] = $row['link'];
    }
 ...
 * @return  array
 */
function assign_reviews($id, $type, $page = 1)
{
    $count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('comment').
           " WHERE id_value = '$id' AND comment_type = '$type' AND status = 1 AND parent_id = 0 AND mod_type = 1");
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['comment_id']]['is_buy']    = $row['is_buy'];
        ...
        $arr[$row['comment_id']]['reply'] = getComments_reply($row['comment_id'], 1);
        $arr[$row['comment_id']]['photos'] = getCommentsPhotos($row['comment_id'],1);
    ...
    $pager['page_count']   = $page_count;
    $pager['page_first']   = "javascript:gotoReviewPage(1,$id,$type)";
    $pager['page_prev']    = $page > 1 ? "javascript:gotoReviewPage(" .($page-1). ",$id,$type)" : 'javascript:;';
    $pager['page_next']    = $page < $page_count ? 'javascript:gotoReviewPage(' .($page + 1) . ",$id,$type)" : 'javascript:;';
    $pager['page_last']    = $page < $page_count ? 'javascript:gotoReviewPage(' .$page_count. ",$id,$type)"  : 'javascript:;';

    return array('reviews' => $arr, 'pager_reviews' => $pager, 'total_review'=>$count);
}
...
{
    $count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('comment').
           " WHERE id_value = '$id' AND comment_type = '$type' AND status = 1 AND parent_id = 0 AND mod_type = 0");
    $size  = !empty($GLOBALS['_CFG']['comments_number']) ? $GLOBALS['_CFG']['comments_number'] : 5;
    $page_count = ($count > 0) ? intval(ceil($count / $size)) : 1;
    $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('comment') .
            " WHERE id_value = '$id' AND comment_type = '$type' AND status = 1 AND parent_id = 0 AND mod_type = 0 ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        //$arr[$row['comment_id']]['rank']     = $row['comment_rank'];
        $arr[$row['comment_id']]['add_time'] = local_date($GLOBALS['_CFG']['time_format'], $row['add_time']);
        $arr[$row['comment_id']]['reply'] = getComments_reply($row['comment_id'],0);
        $arr[$row['comment_id']]['photos'] = getCommentsPhotos($row['comment_id'],0);
    ...
    return array('comments' => $arr, 'pager' => $pager, 'total_comment'=>$count);
}

/* Dùng chung */
function getCommentsPhotos($id, $mod_type = 0){
    $sql = 'SELECT comment_id, img FROM ' . $GLOBALS['ecs']->table('comment_photo') .
                " WHERE comment_id  = $id AND is_hide = 0 AND mod_type = $mod_type ";
    return $GLOBALS['db']->getAll($sql);
}
/* Dùng chung */
function getComments_reply($parent_id, $mod_type = 0){
    $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('comment') .
                " WHERE parent_id  = $parent_id AND mod_type = $mod_type ";
        ...
        while ($row = $GLOBALS['db']->fetch_array($res))
        {
            ...
            $arr[$row['comment_id']]['re_photos'] = getCommentsPhotos($row['comment_id'],0);
        }
    return $arr;
}
