...
class cls_image
{
    ...
    var $bgcolor     = '';
    var $type_maping = array(1 => 'image/gif', 2 => 'image/jpeg', 3 => 'image/png', 4 => 'image/webp');
    var $image;
    var $image_type;
    ...
    function upload_image($upload, $dir = '', $img_name = '')
    {
        ...
        /* 允许上传的文件类型 */
        $allow_file_types = '|WEBP|GIF|JPG|JEPG|PNG|BMP|SWF|';
    ...
    function make_thumb($img, $thumb_width = 0, $thumb_height = 0, $path = '', $bgcolor='', $filename= '')
    {
        ...
        /* check chiều cao, rộng có điền ko */
        ...
        /* resize theo chiều cao => set width = 0 */
        if($thumb_width == 0){
            $this->resizeToHeight($thumb_height);
        }
        /* resize theo chiều rộng => set height = 0 */
        elseif ($thumb_height == 0) {
            $this->resizeToWidth($thumb_width);
        }else{
            $this->resize($thumb_width,$thumb_height, 'n');
        }
        ...
        $new_name = $this->save($dir.$filename,99);
        ...
        {
            return str_replace(ROOT_PATH.CDN_PATH.'/', '', $new_name);
        }
        else
        {
    ...
    static function unique_name($dir)
    {
        ...
        while (empty($filename))
        {
            ...
            if (file_exists($dir . $filename . '.jpg') || file_exists($dir . $filename . '.gif') || file_exists($dir . $filename . '.png') || file_exists($dir . $filename . '.webp'))
            {
                $filename = '';
            }
        }
    ...
    function img_resource($img_file, $mime_type)
    {
        switch ($mime_type)
        {
            ...
            case 4:
            case 'image/webp':
                $res = imagecreatefromwebp($img_file);
                break;
