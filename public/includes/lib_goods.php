 ...
 * @return  array
 */
function get_categories_tree($cat_id = 0, $ext_where = '')
{
    if ($cat_id > 0)
    {
    ...
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT cat_id,cat_name,icon, thumb, parent_id,is_show ' .
                'FROM ' . $GLOBALS['ecs']->table('category') .
                "WHERE parent_id = '$parent_id' AND is_show = 1 $ext_where ORDER BY sort_order ASC, cat_id ASC";
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.seller_note, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['comment_rank']= ceil($comments['comment_rank']);
...
function get_goods_info($goods_id)
{
    ...
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand, m.type_money AS bonus_money, ' .
            ...
            'LEFT JOIN ' . $GLOBALS['ecs']->table('comment') . ' AS r '.
                'ON r.id_value = g.goods_id AND comment_type = 0 AND r.parent_id = 0  AND r.mod_type = 1 AND r.status = 1 ' .
    ...
    {
        /* 用户评论级别取整 */
        $row['comment_rank']  = ceil($row['comment_rank']) == 0 ? 1 : ceil($row['comment_rank']);

        /* 获得商品的销售价格 */
        $row['market_price']        =$row['market_price'] > 0 ? price_format($row['market_price']) : '';
...
function get_goods_properties($goods_id)
{
    ...
    /* 获得商品的规格 */
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, a.extra_type, ".
    ...
    foreach ($res AS $row)
    {
        ...
        if ($row['attr_type'] == 0)
        {
            ...
            $arr['pro'][$group][$row['attr_id']]['value_link'] = $row['display_value'];
        }
        elseif($row['attr_type'] == 4){
            $_key = $row['extra_type'];
            $arr['extra'][$_key]['value'] = $row['attr_value'];
            $arr['extra'][$_key]['name']  = $row['attr_name'];
            $arr['extra'][$_key]['id']    = $row['attr_id'];
            $arr['extra'][$_key]['key']   = $_key;
            $arr['extra'][$_key]['link'] = $row['display_value'];
        }
       
        elseif($row['attr_type'] == 1 || $row['attr_type'] == 2 || $row['attr_type'] == 5)
        {
            //$full_price = $final_price+intval($row['attr_price']);
            $full_price = $final_price;
            /* Lấy thông tin dựa vào ID điền vào display_value */
            if($row['display_type'] == 'links' && !empty($row['display_value'])){
                $goods_id = $row['display_value'];
                $goods_info = goods_info($row['display_value']);
                $full_price = $goods_info['price_final'];
                $row['display_value'] = $goods_info['url'];
            ...
            $arr['spe'][$row['attr_id']]['values'][] = array(
                                                        ...
                                                        'format_price' => $full_price == 0? 'Liên Hệ' : price_format($full_price),
    ...
    if (!empty($attr))
    {
        foreach ($attr['lnk'] AS $key => $val)
        {
            ...
            foreach ($res AS $row)
            {
                ...
                $lnk[$key]['goods'][$row['goods_id']]['market_price']  = $row['market_price'] > 0 ? price_format($row['market_price']) : '';
...
function assign_cat_goods($cat_id, $num = 0, $from = 'web', $order_rule = '')
{
    ...
    /* 分类信息 */
    $sql = 'SELECT cat_name, custom_name, ads_home, ads_home_mobile FROM ' . $GLOBALS['ecs']->table('category') . " WHERE cat_id = '$cat_id'";
...
function get_goods_fittings($goods_list = array())
{
    ...
    {
        $comments = getRank($row['goods_id']);
        $arr[$temp_index]['comment_rank']= ceil($comments['comment_rank']);
