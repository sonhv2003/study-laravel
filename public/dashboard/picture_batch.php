...
function process_image($page = 1, $page_size = 100, $type = 0, $thumb= true, $watermark = true, $change = false, $silent = true)
{
    if ($type == 0)
    {
        $sql = "SELECT g.goods_id, g.goods_name, g.original_img, g.goods_img, g.goods_thumb FROM ".$GLOBALS['ecs']->table('goods'). " AS g WHERE g.original_img > ''" . $GLOBALS['goods_where'];
        $res = $GLOBALS['db']->SelectLimit($sql, $page_size, ($page-1)*$page_size);
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
            $goods_thumb = '';
            $image = '';
            $name_seo  = build_slug($row['goods_name']);
            ...
            if ($watermark)
            {
                ...
                {
                    $dir = dirname(ROOT_PATH.CDN_PATH.'/'. $row['original_img']) . '/';
                }
                else
                {
                    $dir = dirname(ROOT_PATH.CDN_PATH.'/'. $row['goods_img']) . '/';
                }

                $image = $GLOBALS['image']->make_thumb(ROOT_PATH.CDN_PATH.'/'. $row['original_img'], $GLOBALS['_CFG']['image_width'],  $GLOBALS['_CFG']['image_height'], $dir); //先生成缩略图
                ...
                /* Bỏ qua bước này */
                //$image = $GLOBALS['image']->add_watermark(ROOT_PATH.CDN_PATH.'/'. $image, '', $GLOBALS['_CFG']['watermark'], $GLOBALS['_CFG']['watermark_place'], $GLOBALS['_CFG']['watermark_alpha']);
                ...
                /* 重新格式化图片名称 */
                $image = reformat_image_name('goods', $row['goods_id'], $image, 'goods', $name_seo);
            ...
            if ($thumb)
            {
                ...
                {
                    $dir = dirname(ROOT_PATH.CDN_PATH.'/'. $row['goods_thumb']) . '/';
                }

                $goods_thumb = $GLOBALS['image']->make_thumb(ROOT_PATH.CDN_PATH.'/'.$row['original_img'], $GLOBALS['_CFG']['thumb_width'], $GLOBALS['_CFG']['thumb_height'], $dir);
                ...
                /* 重新格式化图片名称 */
                $goods_thumb = reformat_image_name('goods_thumb', $row['goods_id'], $goods_thumb, 'thumb', $name_seo);
    ...
    else
    {
        ...
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
            $thumb_url = '';
            $image     = '';
            $name_seo  = $GLOBALS['db']->getOne("SELECT slug FROM ".$GLOBALS['ecs']->table('goods_gallery')." WHERE id = $row[goods_id] AND module = 'goods'");
            /* 水印 */
            if ($watermark && file_exists(ROOT_PATH.CDN_PATH.'/'. $row['img_original']))
            {
                if (empty($row['img_url']))
                {
                    $dir = dirname(ROOT_PATH.CDN_PATH.'/'. $row['img_original']) . '/';
                }
                else
                {
                    $dir = dirname(ROOT_PATH.CDN_PATH.'/'. $row['img_url']) . '/';
                }

                $file_name  = cls_image::unique_name($dir);
                $file_name .= cls_image::get_filetype(empty($row['img_url']) ? $row['img_original']: $row['img_url']);

                copy(ROOT_PATH.CDN_PATH.'/'. $row['img_original'], $dir . $file_name);
                ...
                /* 重新格式化图片名称 */
                $image = reformat_image_name('gallery', $row['goods_id'], $image, 'goods', $name_seo);
            ...
            if ($thumb)
            {
                ...
                {
                    $dir = dirname(ROOT_PATH.CDN_PATH.'/'. $row['thumb_url']) . '/';
                }

                $thumb_url = $GLOBALS['image']->make_thumb(ROOT_PATH.CDN_PATH.'/'. $row['img_original'], $GLOBALS['_CFG']['thumb_width'], $GLOBALS['_CFG']['thumb_height'], $dir);
                ...
                /* 重新格式化图片名称 */
                $thumb_url = reformat_image_name('gallery_thumb', $row['goods_id'], $thumb_url, 'thumb', $name_seo);
...
function process_image_ex($page = 1, $page_size = 100, $type = 0, $thumb= true, $watermark = true, $change = false, $silent = true)
{
    ...
    if ($type == 0)
    {
        $sql = "SELECT g.goods_id, g.goods_name, g.original_img, g.goods_img, g.goods_thumb FROM ".$GLOBALS['ecs']->table('goods'). " AS g WHERE g.original_img > ''" . $goods_where;
        $res = $GLOBALS['db']->SelectLimit($sql, $page_size, ($page-1)*$page_size);

        while ($row = $GLOBALS['db']->fetchRow($res))
        {
            $name_seo = $name_seo == '' ? build_slug($row['goods_name']) : $name_seo;
            if ($thumb)
            {
