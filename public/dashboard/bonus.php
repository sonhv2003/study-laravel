 ...
 * ----------------------------------------------------------------------------
 * send_by_online
 * Tiêu chí: Dùng cho nhiều người
 * Có giới hạn số lượng
 * Có điều kiện: áp dụng cho sản phẩm, nhản hàng, tổng hóa đơn
 * ============================================================================
 * $Author: liubo $
...
if ($_REQUEST['act'] == 'send')
{
    ...
    elseif ($_REQUEST['send_by'] == SEND_BY_ONLINE)
    {
        $smarty->assign('type_list',    get_bonus_online());
        $smarty->assign('id',           $id);
        $smarty->display('bonus_by_online.htm');
    }
}
...
if ($_REQUEST['act'] == 'send_by_print')
{
    ...
    {
        $bonus_sn = date('Ymd').$i. str_pad(mt_rand(0, 9999), 5, '0', STR_PAD_LEFT);
...
/* chỉ tạo một mã thôi */
if ($_REQUEST['act'] == 'send_by_online')
{
    @set_time_limit(0);

    $bonus_typeid = !empty($_POST['bonus_type_id']) ? $_POST['bonus_type_id'] : 0;
    $bonus_sum    = !empty($_POST['bonus_sum']) && $_POST['bonus_sum'] > 1 ? intval($_POST['bonus_sum'])     : 1;
    $bonus_sn    = !empty($_POST['bonus_sn']) ? trim($_POST['bonus_sn']) : '';

    /* check mã xem bị trùng ko*/
    $sql = 'SELECT COUNT(*) FROM ' .$ecs->table('user_bonus'). " WHERE bonus_sn = '$bonus_sn'";
    $count_bonus_sn = $db->getOne($sql);
    var_dump($bonus_sn);
    if (!$count_bonus_sn == 0)
    {
        sys_msg($_LANG['bonus_exited']);
    }
    
    $db->query("INSERT INTO ".$ecs->table('user_bonus')." (bonus_type_id, bonus_sn, max_quantity) VALUES('$bonus_typeid', '$bonus_sn', $bonus_sum)");
...
if ($_REQUEST['act'] == 'bonus_list')
{
    ...
    elseif ($bonus_type['send_type'] == SEND_BY_ONLINE) {
        $smarty->assign('show_bonus_sn', 1);
    }
