...
if ($_REQUEST['act'] == 'list')
{
    ...
    $curr_template_mobile = $_CFG['template_mobile'];
    $curr_style = $_CFG['stylename'];

    /* Template Desktop */
    $available_templates = array();
    /* Template Mobile */
    $available_templates_mobile = array();
    ...
    while ($file = readdir($template_dir))
    {
        ...
        {
            if(strpos($file,'mobile') > 0){
                 $available_templates_mobile[] = get_template_info($file);
            }else{
    ...
    /* Nhận một mảng kiểu tùy chọn của các mẫu có sẵn */
    ...
    /* Xóa cài đặt mẫu không mong muốn */
    ...
    // $_dir_backup = ROOT_PATH . 'temp/backup/library/';
    // if (!file_exists($_dir_backup))
    // {
    //     @mkdir($_dir_backup, 0777);
    //     @chmod($_dir_backup, 0777);
    // }
    // $tmp_bak_dir = @opendir($_dir_backup);
    // while ($file = readdir($tmp_bak_dir))
    // {
    //     if ($file != '.' && $file != '..' && $file != '.svn' && $file != 'index.htm' && is_file($_dir_backup . $file) == true)
    //     {
    //         $code = substr($file, 0, strpos($file, '-'));
    //         if (!in_array($code, $available_code))
    //         {
    //             @unlink($_dir_backup . $file);
    //         }
    //     }
    // }
    ...
    $smarty->assign('curr_template_mobile', get_template_info($curr_template_mobile, $curr_style));
    $smarty->assign('available_templates', $available_templates);
    $smarty->assign('available_templates_mobile', $available_templates_mobile);
    $smarty->assign('theme_online_url', '');
...
if ($_REQUEST['act'] == 'setup')
{
    ...
    /* Lấy danh sách thư mục themes */
    $themes_list = get_themes_list();
    $smarty->assign('themes_list',    $themes_list);
    /* End Lấy danh sách thư mục themes */

    $template_theme = empty($_REQUEST['template_theme']) ? $_CFG['template'] : $_REQUEST['template_theme'];
    $curr_template  = empty($_REQUEST['template_file']) ? 'index' : $_REQUEST['template_file'];

    /* lấy danh sách thư viện của theme */
    $temp_options   = array();
...
if ($_REQUEST['act'] == 'setting')
{
    ...
    //$curr_template = $_CFG['template'];
    $curr_template = $_POST['theme']; //edit by Nobjnguyen
    $filename = $_POST['template_file'];
    $db->query("DELETE FROM " .$ecs->table('template'). " WHERE remarks = '' AND filename = '$filename' AND theme = '$curr_template'");
...
if ($_REQUEST['act'] == 'library')
{
    ...
    $curr_template = empty($_REQUEST['template_theme']) ? $_CFG['template'] : $_REQUEST['template_theme'];
...
if ($_REQUEST['act'] == 'install')
{
    ...
    if($is_mobile == 0){
        ...
        $sql = "UPDATE " .$GLOBALS['ecs']->table('shop_config'). " SET value = '$tpl_name' WHERE code = 'template_mobile'";
    }
...
{
    $cur_themes = trim($_GET['themes']); //add by Nobj
    $library = load_library($cur_themes, trim($_GET['lib']));
...
if ($_REQUEST['act'] == 'update_library')
{
    ...
    $cur_themes = trim($_REQUEST['themes']); //add by Nobj
    $lib_file = '../themes/' . $cur_themes . '/library/' . $_POST['lib'] . '.lbi';
    ...
    {
        @file_put_contents('../temp/backup/library/' . $cur_themes . '-' . $_POST['lib'] . '.lbi', $org_html);
    ...
    {
        make_json_error(sprintf($_LANG['update_lib_failed'], 'themes/' . $cur_themes . '/library'));
...
if ($_REQUEST['act'] == 'restore_library')
{
    ...
    $lib_name   = trim($_GET['lib']);
     $cur_themes = trim($_REQUEST['themes']); //add by Nobj
    $lib_file   = '../themes/' . $cur_themes . '/library/' . $lib_name . '.lbi';
    $lib_file   = str_replace("0xa", '', $lib_file); // 过滤 0xa 非法字符
    $lib_backup = '../temp/backup/library/' . $cur_themes . '-' . $lib_name . '.lbi';
...
if ($_REQUEST['act'] == 'backup_setting')
{
    admin_priv('backup_setting');
      ...
      /* Lấy danh sách thư mục themes add by Nobj */
    $themes_list = get_themes_list();
    $smarty->assign('themes_list',    $themes_list);
    /* End Lấy danh sách thư mục themes */
...
{
    /* add by Nobj */
    $_CFG['template'] = isset($_REQUEST['template_theme']) ? trim($_REQUEST['template_theme']) : $_CFG['template'];
...
function get_themes_list(){
    ...
    {
        if ($file != '.' && $file != '..' &&  $file != '.svn' && $file != '_svn' && is_dir(ROOT_PATH.'themes/' .$file))
        {
            $themes_list[] = $file;
        }
    }
