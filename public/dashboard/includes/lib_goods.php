...
function handle_link_goods($goods_id)
{
    $sql = "UPDATE " . $GLOBALS['ecs']->table('link_goods') . " SET " .
            " goods_id = '$goods_id' " .
            " WHERE goods_id = '0' AND module = 0 " .
            " AND admin_id = '$_SESSION[admin_id]'";
    $GLOBALS['db']->query($sql);

    $sql = "UPDATE " . $GLOBALS['ecs']->table('link_goods') . " SET " .
            " link_goods_id = '$goods_id' AND module = 0  " .
...
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls, $filename ='',$sort=1,$type=1)
{
    ...
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        if ($flag)
        {
            ...
            /* Tạo thumb cho gallery hình sản phẩm */
            ...
            /* Tạo thumb cho gallery tính năng, ảnh bia */
            elseif ($proc_thumb && $type[$key] > 1 && $GLOBALS['_CFG']['build_thumb_gallery'] == true) {
                $thumb_url = $GLOBALS['image']->make_thumb($image_files['tmp_name'][$key], $GLOBALS['_CFG']['gallery_thumb_width'],  $GLOBALS['_CFG']['gallery_thumb_height']);
                $thumb_url = is_string($thumb_url) ? $thumb_url : '';
            }else{
                $thumb_url = '';
            }
            ...
            elseif ($type[$key] > 1 && $GLOBALS['_CFG']['build_thumb_gallery'] == true) {
                $thumb_url = reformat_image_name('gallery_thumb', $goods_id, $thumb_url, 'thumb', $filename);
            }else{
                 $thumb_url = '';
            }
...
function delete_goods($goods_id)
{
    ...
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('link_goods') . " WHERE module = 0 AND goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('link_goods') . " WHERE module = 0 AND link_goods_id " . db_create_in($goods_id);
    $GLOBALS['db']->query($sql);
...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.display_type, v.display_value, v.attr_value, v.attr_price ".
            ...
            "ORDER BY a.attr_type, a.sort_order, a.attr_id, v.attr_price, v.goods_attr_id";
...
function build_attr_html($cat_id, $goods_id = 0)
{
    ...
    foreach ($attr AS $key => $val)
    {
        ...
        if($val['attr_input_type'] == 1){
           $html .= $val['attr_type'] == 3 ? "<span style='color:#f60'>".$val['attr_name']."</span>" : "<span style='color:#41a5e1'>".$val['attr_name']."</span>";
        }else{
          $html .= $val['attr_name'];
        }
        $html .= "</td><td><input type='hidden' name='attr_id_list[]' value='$val[attr_id]' />";

        if ($val['attr_input_type'] == 0)
        {
            if($val['attr_type'] == 5){
                $html .= 'Form cho khách nhập <input name="attr_value_list[]" type="hidden" value="' .$val['attr_name']. '" size="40" /> ';
            }else{
       ...
       if($val['attr_type'] == 1 || $val['attr_type'] == 2){
        ...
        /* Định nghĩa kiểu hiển thị các input mở rộng */
       // elseif($val['attr_type'] == 4){

       //      $html .= $GLOBALS['_LANG']['display_type'].' <select name="display_type_list[]">
       //                  <option value="default">Default</option>';
       //      foreach ($GLOBALS['_LANG']['select_type_extra'] as $key => $row) {
       //          $selected = $key == $val['display_type'] ? 'selected': '';
       //      $html .= $val['display_type'].'<option value="'.$key.'" '.$selected.'>'.$row.'</option>';
       //      }
       //      $html .=   '</select>';
       ...
       if($val['attr_type'] == 5){
            $html .=  ' <input type="hidden" placeholder="Có thể là Link, mã màu..." name="display_value_list[]" value="' . $val['display_value'] . '" size="30" />';
        }else{
            $html .=  $GLOBALS['_LANG']['display_value'].' <input type="text" placeholder="Có thể là Link, mã màu..." name="display_value_list[]" value="' . $val['display_value'] . '" size="30" />';
        }
...
/**
 * Lấy tin liên quan cho sản phẩm
 *
 * @access  public
 * @param   integer $goods_id
 * @return  array
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT goods_id, goods_name, goods_type, goods_sn, virtual_sales, is_home, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, " .
    ...
    foreach ($row as $rows) {
        ...
        /* update tồn kho sp ảo */
        if($real_goods == 0){
            $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('virtual_card')." WHERE goods_id = '$rows[goods_id]' AND is_saled = 0 ";
            $num = $GLOBALS['db']->GetOne($sql);
            
            $GLOBALS['db']->query("UPDATE ".$GLOBALS['ecs']->table('goods')." SET goods_number = $num WHERE goods_id = '$rows[goods_id]' LIMIT 1");
            
            $rows['goods_number'] = $num;
        }
...
function reformat_image_name($type, $goods_id, $source_img, $position='', $filename = '')
{
    ...
    if ($position == 'source')
    {
        ...
        {
            return 'images/'.$sub_dir.'/source_img/'.$img_name.$img_ext;
        }
    }
    elseif ($position == 'thumb')
    {
        ...
        {
            return 'images/'.$sub_dir.'/thumb_img/'.$img_name.$img_ext;
        }
    }
    else
    {
        ...
        {
            return 'images/'.$sub_dir.'/goods_img/'.$img_name.$img_ext;
        }
    }
    return false;
}
