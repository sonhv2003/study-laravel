...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    elseif ($_REQUEST['step'] == 2)
    {
        ...
        $item_url = isset($_REQUEST['item_url']) ? $_REQUEST['item_url'] : '';
        $item_ifshow = isset($_REQUEST['item_ifshow']) ? $_REQUEST['item_ifshow'] : '';
        $item_opennew = isset($_REQUEST['item_opennew']) ? $_REQUEST['item_opennew'] : '';
        $item_type = isset($_REQUEST['item_type']) ? $_REQUEST['item_type'] : '';
        /* add by Ecshopvietnam */
        $item_class = isset($_REQUEST['item_class']) ? $_REQUEST['item_class'] : '';
        $item_rel = isset($_REQUEST['item_rel']) ? $_REQUEST['item_rel'] : '';
        $item_icon = isset($_REQUEST['item_icon']) ? $_REQUEST['item_icon'] : '';
        ...
        if($item_ifshow == 1 && $item_type == 'middle')
        {
            ...
            if($arr)
            {
                ...
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('nav') . " (name,ctype,cid,ifshow,vieworder,opennew,url,type,class,icon,rel) VALUES('$item_name','".$arr['type']."','".$arr['id']."','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type', '$item_class', '$item_icon', '$item_rel')";
            }
        }

        if(empty($sql))
        {
            $sql = "INSERT INTO " . $GLOBALS['ecs']->table('nav') . " (name,ifshow,vieworder,opennew,url,type,class,icon,rel) VALUES('$item_name','$item_ifshow','$item_vieworder','$item_opennew','$item_url','$item_type','$item_class', '$item_icon', '$item_rel')";
        }
        $db->query($sql);
        clear_cache_files();
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    if (empty($_REQUEST['step']))
    {
        ...
        $rt['item_opennew_'.$row['rel']] = 'selected';
        $rt['item_class'] = $row['class'];
        $rt['item_icon'] = $row['icon'];
    ...
    elseif ($_REQUEST['step'] == 2)
    {
        ...
        if($arr)
        {
            ...
            $sql = "UPDATE " . $GLOBALS['ecs']->table('nav') .
                " SET name='$item_name',ctype='" . $arr['type'] . "',cid='" . $arr['id'] . "',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',type='$item_type', class='$item_class', icon='$item_icon', rel='$item_rel' WHERE id='$id'";
        }
        else
        {
            //目标不是分类
            ...
            $sql = "UPDATE " . $GLOBALS['ecs']->table('nav') .
                " SET name='$item_name',ctype='',cid='',ifshow='$item_ifshow',vieworder='$item_vieworder',opennew='$item_opennew',url='$item_url',type='$item_type', class='$item_class', icon='$item_icon', rel='$item_rel' WHERE id='$id'";
        }
...
function get_sysnav()
{
    ...
    $sysmain = array(
        ...
        array($_LANG['group_buy_goods'],'nhom-mua'),
        array($_LANG['snatch'],'dau-gia'),
        array($_LANG['tag_cloud'],'tag-cloud'),
        array($_LANG['user_center'],'thanh-vien'),
        array($_LANG['wholesale'], 'ban-si'),
        array($_LANG['activity'], 'uu-dai'),
        array($_LANG['message_board'], 'lien-he')
        );
...
/*------------------------------------------------------ */
//-- Phan tich dinh dang url de xac dinh module nao
/*------------------------------------------------------ */
